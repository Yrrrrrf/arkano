...
void (async () => {
  await new Command()
    ...
    .option('-i, --in <dir:string>', 'Input directory containing .svelte components', {
      default: './fixtures/components',
    })
    .option('-o, --out <dir:string>', 'Output directory for generated adapters', {
      default: './dist/adapters',
    })
    .option('-t, --target <framework:string>', "Target framework: 'react', 'vue', or 'all'", {
      default: 'all',
    })
