 ...
 */
export function createPropValidator<T extends Record<string, unknown> = Record<string, unknown>>(
  schemaDefinition: object,
): (props: unknown) => props is T {
  ...
  return (props: unknown): props is T => {
    ...
    if (out instanceof type.errors) {
      console.warn(`[Arkane Prop Mismatch] Component received invalid props:\n${out.summary}`);
      return false;
    }
    return true;
  };
